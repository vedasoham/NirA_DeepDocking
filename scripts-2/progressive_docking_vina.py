        ...
        for line in ref:
            ...
            if (tmp[0] in Oversampled_zid.keys()) and (type(Oversampled_zid[tmp[0]]) != np.ndarray):
                ...
                for elem in on_bit_vector:
                	if elem:  # Check if elem is not an empty string
...
if (y_valid_first.VINA_Score < cf).values.sum() <= 10 or \
        (y_test_first.VINA_Score < cf).values.sum() <= 10:
...
if CONTINUOUS:
    ...
    y_valid = valid_data.VINA_Score
    y_test = test_data.VINA_Score
    y_train = train_data.VINA_Score
    ...
    y_valid = y_valid_first.VINA_Score < cf
    y_test = y_test_first.VINA_Score < cf
    y_train = train_data.VINA_Score < cf
